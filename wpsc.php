...
function update()
{
  ...
  echo "\n\e[91m\e[1m[+] ALAT PEMBARUAN WPSC [+]\nProses pembaruan berlangsung, harap tunggu...\n\n$cln";
  system("git fetch origin && git reset --hard origin/master && git clean -f -d");
  echo $bold . $fgreen . "[i] Pembaruan selesai! Silakan jalankan ulang WPSC.\n" . $cln;
  exit;
}
...
if (extension_loaded('curl') || extension_loaded('dom'))
...
else
  {
    if (!extension_loaded('curl'))
      {
        echo $bold . $red . "\n[!] Modul cURL belum terinstal! Gunakan perintah 'fix' atau instal php-curl" . $cln;
      }
...
thephuckinstart:
echo "\n";
userinput("Masukkan Website yang ingin di-scann ");
...
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] MENU PERBAIKAN [+]\n\n$cln";
    echo $bold . $blue . "[+] Checking If cURL module is installed ...\n";
    if (!extension_loaded('curl'))
      {
        echo $bold . $red . "[!] Modul cURL belum terinstal! Menginstal... \n";
        ...
        echo $bold . $fgreen . "[i] cModul cURL berhasil diinstal \n";
      }
    else
      {
        echo $bold . $fgreen . "[i] Modul cURL sudah terinstal \n";
      }
    ...
    echo $bold . $fgreen . "[i] Job finished successfully! Please Restart WPSC \n";
    exit;
  }
elseif ($ip == "update")
  {
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want WPSC To Detect CMS Of The Sites? [Y/N]");
            ...
            if (strpos($srccd, "wp-content") !== false)
              {
                ...
                if ($wp_version != "")
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] WPSC could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ WPSC Update Corner]-\n\n" . $cln;
            ...
            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of WPSC. \n\n" . $cln;
                goto scanlist;
              }
            else
              {
